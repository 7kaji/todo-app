...
class CognitoJwtVerifier
  ...
  ISS = "https://cognito-idp.ap-northeast-1.amazonaws.com/#{ENV['COGNITO_USER_POOL_ID']}" # https://cognito-idp.{region}.amazonaws.com/{userPoolId}
  TOKEN_USE_CLAIM = 'id'

  def initialize(token)
    @token = token
    @keys = raw_keys_json['keys']
  end
  ...
  def validate_and_decrypt()
    ...
    public_key = JOSE::JWK.from_map(@keys[key_index].to_hash) # Get the last 2 sections of the token, payload and signature.
  ...
  private
    ...
    def raw_keys_json
      Rails.cache.fetch("coginito_jwks", expired_in: 1.hour) do
        # Raw keys retrieved from
        uri = URI.parse("https://cognito-idp.ap-northeast-1.amazonaws.com/#{ENV['COGNITO_USER_POOL_ID']}/.well-known/jwks.json")
