...
const Todos: React.FC = () => {
  ...
  const getTodos = async (page: string) => {
    ...
    await axios
      .get(`/api/v1/todos${pageParamsString}`)
      .then(response => {
        ...
        setTotalPage(
          Math.ceil(
            Number(response.headers["x-total"]) / response.headers["x-per-page"]
          )
        );
  ...
  const createTodo = (e: React.KeyboardEvent<HTMLDivElement>): void => {
    if (e.key === "Enter" && !(inputValue === "")) {
      axios
        .post("/api/v1/todos", { todo: { title: inputValue } })
        .then(response => {
      ...
      <div className="paginationCenter">
        <div className="pagination">
          {currentPage !== "1" && (
            <Button
              variant="outlined"
              onClick={() => handlePage(prevPage.page)}
            >
              prev
            </Button>
          )}
          ...
          {nextPage.page !== "-1" && (
            <Button
              variant="outlined"
              onClick={() => handlePage(nextPage.page)}
            >
              next
            </Button>
          )}
